...
class Renderer:
    ...
    def __init__(self, cards: List[Card], **kwargs):
        ...
        self.face_up_background: str = kwargs.get(
            "face_up_background", "card_front_sprites_24x24.bmp"
        )
        self.face_down_background: str = kwargs.get(
            "face_down_background", "card_back_sprites_24x24.bmp"
        )
        self.symbol_image: str = kwargs.get("symbol_image", "card_symbols_24x24.bmp")
        self.background_tile_size: int = kwargs.get("background_tile_size", 24)
        ...
        self.symbol_tile_mapping: SymbolTileMap = kwargs.get("symbol_tile_map", DEFAULT_SYMBOL_MAP)
    ...
    def load_assets(self) -> None:
        ...
        self.face_up_bmp, self.face_up_palette, self.face_up_tilegrid = self.load_bitmap_from_disk(
            self.face_up_background, 3, 3, self.background_tile_size
        )

        (
            self.face_down_bmp,
            self.face_down_palette,
            self.face_down_tilegrid,
        ) = self.load_bitmap_from_disk(self.face_down_background, 3, 3, self.background_tile_size)

        self.symbol_bmp, self.symbol_palette, self.symbol_tilegrid = self.load_bitmap_from_disk(
            self.symbol_image, 10, 4, self.symbol_tile_size
        )

    def load_bitmap_from_disk(
        self, bitmap_name: str, x_tiles: int, y_tiles: int, tile_size: int
    ) -> Tuple[displayio.Bitmap, displayio.Palette, displayio.TileGrid]:
        """
        ...
        sprite_sheet, palette = adafruit_imageload.load(
            bitmap_path, bitmap=displayio.Bitmap, palette=displayio.Palette
        )
